...
* `echo = FALSE` - Execute the code and display the results, but don't put the code into the final document.  
* `message = FALSE` - Don't display any messages R tells you as a result of running code.  Very useful with `ggplot()` when specifying `geom_smooth(method = lm)`
...
x_1 <- 55
```
Any object you create should show up in your environment window. Let's clear our enviromnemt again:
```{r}
rm(list=ls())
```
...
class(union)
```
What went wrong with the `class(union)` command?  It turns out there is a function in base R called `union`, so R is not sure what you are referring to.  This is a case where you are stuck using \$ notation, even though we used `attach()` on the data set.
