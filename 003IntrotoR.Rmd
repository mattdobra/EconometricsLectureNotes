...
By using the # in your code, you can put comments in your code that R will not execute but will tell you what is going on.  
...
Will install the Tidyverse package onto your computer.  Once it is installed, you need to load it into your session whenever you use it.  When scripting, it is a good idea to load in any packages early in your script.  For example, nearly everything I write includes the following line toward the very top of the script:

```{r}
library(tidyverse)
```

I wind up needing more packages as I develop the script, I try to put their library call at the beginning of the script, not at the end.  For example, in my R scripts that I use to write my Principles of Macroeconomics course notes, I have the following code in lines 13-23, as I wind up using these packages in nearly every script.
...
Let's talk about 2 script types here, basic scripts and *rmarkdown* documents.  An R script is used to create (and recreate!) a series of commands in R.  An *rmarkdown* document incorporates script elements into a high quality document that can be shared in a variety of formats.
...
q
```
Something you will probably strugge a lot with early on is coping with the fact that R takes being **case sensitive** VERY, VERY seriously!
```{r, error = TRUE}
Q
```
...
rm(list=ls())
```
objects can include numbers, periods, or underscores but must begin with a letter.
```{r}
x.1 <- 44
x_1 <- 55
```
...
There are also many packages that are full of data, especially in support of various textbooks. For example:
```{r, eval = FALSE}
...
library(AER)
```
Will install a package with the data sets that accompany a couple textbooks.  Now, a *data()* call will list a lot more available databases!

Let's grab the CPS1985 dataset from the AER package:
```{r}
data(CPS1985)
```
...
There are many other utilities for importing data into R, including packages such as *readxl*, *haven*, *Hmisc*, *foreign*.  If the data exists in a somewhat common format, somebody has written the package to import it into R!
...
The Wehde textbook only looks at the Wave 12 (Fall 2016) data, so let's filter that out using the *dplyr* tools we saw in our first lesson.
...
```
You may want to keep this data set for later (you can use it as you read through the Wehde et. al. text to replicate their work), so let's save it as a csv file called Oklahoma.csv
...
We can also learn about about what the data generally look like byusing the *head* function:
```{r}
head(CPS1985)
```
We can also use square brackets to subset bits of data.  Remember, brackets use the rows, columns convention.  Here I look at rows 222-225.  I leave the column part empty so I get all of the columns
...
CPS1985 %>% 
    ...
    select(-gender)
```
Sometimes subsetting is the better approach, sometimes dplyr is better, it just depends on what you are trying to accomplish.  Both have their place:
...
Sometimes, datasets will code a categorical variable as a number.  Here, I will code a variable called female which will be 1 for females and 0 for males.
