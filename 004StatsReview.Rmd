...
Before we get started, a brief note on graphing.  We will be using both Base R graphics and `ggplot()` graphics. In my opinion, `ggplot()` makes better looking graphics and is more intuitive when you want to customize your graphs than Base R.  But sometimes, you just want a quick and dirty graph, and Base R is just fine for that.  Thsi notebook will use both from time to time. Also, I won't be wasting a lot of time making every graph look "good" here.  A good looking graph needs to have a title, axis titles, legends, and so forth.   Sometimes I will put in the effort to make the graph look proper when it is a `ggplot()` graph, sometimes I won't. Data visualization is well beyond the scope of this course.
...
* **Layers** - Objects are created on the aesthetic in layers.  Each layer includes, or is define by:
...
Histograms are useful for displaying the distribution of continuous data.  Again, much easier to do in R than in something like Excel. Here is the distribution of respondent age in the CPS1985 data set using the `hist()` function.
