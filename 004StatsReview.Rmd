...
This notebook will provide an overview of basic statistics.  This is not meant to replace the full-semester course that you should have already taken, but rather to demonstrate how to do the techniques learned in that class using R.

Before we get started, a brief note on graphing.  We will be using both Base R graphics and *ggplot2* graphics. In my opinion, *ggplot2* makes better looking graphics and is more intuitive when you want to customize your graphs than Base R.  But sometimes, you just want a quick and dirty graph, and Base R is just fine for that.  So I will use both from time to time. Also, I won't be wasting a lot of time making the graphs "good" here.  A good looking graph needs to have a title, axis titles, legends, and so forth.   Sometimes I will put in the effort to make the graph look proper when it is a *ggplot* graph, sometimes I won't.  Data visualization is well beyond the scope of this course.   

Finally, before we get going, let's load in the packages we will be needing and load and attach some data:
...
The package *ggplot2* is based on the idea of the Grammar of Graphics.  The Grammar of Graphics is based on the idea that a graphic has a set of indispensable elements:

* Aesthetic - The dimensions and visual attributes of the graph.  This can be the x/y dimension, but also you can graph using color, size, etc.
* Layers - Objects are created on the aesthetic in layers.  Each layer includes
...
This is some higher level data visualization stuff that is beyond the scope of this class.  Still, understanding a bit of this is helpful to making good graphics with *ggplot2*

# Basic Graphing Techniques

## Bar and Pie Charts

Let's start with bar charts and pie charts.  For basic bar charts, I find *ggplot* much easier.  Here is a bar chart of the CPS data by occupation:
```{r}
...
* Adding the `+` allows me to break up my code over multiple lines
* geom_bar() creates a basic bar chart.

This can be made more proper very quickly:
```{r}
CPS1985 %>% ggplot(aes(x = occupation))+
...
```
Bar charts in Base R require more work.  First, you need to create an object with the relevant counts:
```{r}
jobtype <- table(occupation)
jobtype
```
This transformed the data into a table of counts.  Now, we can graph this object with a barplot
```{r}
barplot(jobtype)
```
This plot is also customizable in many of the say ways the ggplot one was if you want to look into it.

We can also display this sort of data in a pie chart.  Fun fact: most people who study data visualization believe that a pie chart is a really awful way to graph data.  This may be why *ggplot2* doesn't have an easy way of making a pie chart.  However, you can make a pie chart in base R very easily with:

```{r}
pie(jobtype)
```
I am on team "Pie Charts are evil" so that's all I will say about them.

## Boxplots

Boxplots are a very common way of displaying the distribution of numerical data by showing the 0th, 25th, 50th, 75th, and 100th quartile of data on a graph. I usually skip them when I teach intro to stats because they are stupidly hard to make in Excel.  But they are a snap in R.  Here is base R:
...
```
This seems to be missing the label for management class, we'd have to do soem work to put it in there.  But I'd do this in *ggplot* anyway, so:
```{r}
...
Histograms are useful for displaying the distribution of continuous data.  Again, much easier to do in R than in something like Excel. Here is the distribution of respondant age in the CPS1985 data set.
...
```
Note that with *ggplot*, usually *color* is used for outlines and *fill* is used to color in spaces.  Let's make our graph with the MU official colors:
* MU Green (#00573C)
...
CPS1985 %>% ggplot(aes(x = occupation, fill = gender)) +
    geom_bar()
```
Going to switch the colors.  Gender stereotype colors may enhance readability to the viewer, but if you want to avoid them, then you probably don't want to do the exact opposite of what people expect.
...
Let's look at the relationship between the share of campaign expenditures (shareA) and vote share received (voteA).  Base R makes a simple scatter plot with the plot function.  The syntax is `plot(y ~ x)`
```{r}
plot(voteA ~ shareA)
```
...
All in all, R has some extremely powerful graphing capabilities that go well beyond what something like Excel is capable of.  It's no surprise that many major news networks use R, and particularly *ggplot2* to do their graphics. For example, the BBC and the New York Times produce many of their graphics using ggplot2.  In fact, the BBC has even distributed a package you can download called *bbplot* that you can install (using *devtools* unfortuantely) make your graphs look like BBC graphics! 
...
Arithmetic means are calcualted with the *mean()* function.
```{r}
mean(wage)
```
...
This notebook was essentially the first half of an intro to statistics class.  The next step is to tackle inferential statistics using R.