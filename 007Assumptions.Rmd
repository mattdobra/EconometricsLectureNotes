...
We will work through a series of assumptions upon which the OLS model is built, and what one might do if these assumptions do not hold. We will be encountering these assumptions throughout the semester!
...
Now, let's take a look at the plot between lnsubs and lnprice:
...
```
Wow, quite a change.  This data looks like it might actually have a *linear* relationship.
...
The model in column 4 seems to be the best model of the bunch.  We will look at other non-linear transformations as we move forward in the class.
...
Typically this error is violated when you have **confounding** variables or **omitted variables**, which basically means you need to add more independent variables to your model.  We will discuss this idea more when we get into regression with more than one independent variable.

## Assumption 4: The error term is not serially correlated.

This is mostly a problem in time series regression, and refers to a situation in which you can use error terms to predict each other.  Let's take a look at this data from the New York Stock Exchange from the wooldridge package--these data are about 12 years of Wednesday closing prices of the NYSE.  This data includes a pretty big market crash around observation 620 or so, which leads me to speculate that this data is from the 1980s and the huge drop is in October, 1987.  
...
tempdata2 %>% ggplot(aes(x = t, y = resid)) +
    ...
    geom_line()
```
This graph exhibits what is called *serial correlation* or *autocorrelation* as the error terms are correlated with each other.  In other words, if we know the error term for week 1, we can use that to make a pretty good gess about the error term for week 2, and so forth. Again, the issues of serial correlation are mostly time series issues, so if we get to time series at the end of the semester, we will discuss these issues more then.
...
## Assumption 6: No indepentent variable is perfect linear function of other explanatory variables.

This one is important and will probably create quite a few headaches for you when we get to dummy variable regression.  Let's introduce the concept quickly here though.  

Ordinary least squares works by attributing the variation in the dependent variable Y to the variation in your independent variables.  If you have more than one independent variable, OLS needs to figure out which independent variable to attribute the variation to.  If you have two identical independent variables, R cannot distinguish one variable from the other when trying to apportion variation.  If you attempt to estimate a model that contains independent variables that are perfectly correlated, R will spit out an error message.  Typically, the way to proceed is to simply remove one of the offending variables.  
...
summary(reg6a)
```
R doesn't even let me run this.  So let's trick it into running a regression:

```{r}
tempdata3 <- vote1
tempdata3$shareAclone <- tempdata3$sharaA
...
* $X_1$ cannot be reached with a linear combination of  $X_2$ and $X_3$. So, if $X_1$ is income, $X_2$ is consumption, and $X_3$ is savings, so $X_1 = X_2 + X_3$, you can't include all 3 variables in your equation. This is true of more compicated linear combinations as well; if $X_1 = 23.1 + .2X_2 - 12.4X_3$, you couldn't run that either.  

Why this typically causes trouble for people, however, is that they are not usually aware that there is another variable hidden in the regression, $X_0$, which carries a value of 1 for every observation.  This is technically what the $\alpha$ is multiplied by.  This means that $X_1$, $X_2$, and $X_3$ **cannot** be constants, because otherwise you will violate this assumption. 
