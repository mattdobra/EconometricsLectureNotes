...
Let's start with a bit of a whirlwind introduction to R using the Gapminder data.  The focus today will be on doing stuff, rather than understanding how the code works.  
...
Another useful tool for creating tables is the *stargazer* package. It's a bit finicky in this instance, but the result is a nicely formatted table.
```{r}
...
```
If you are feeling lazy and don't want to have to type the name of the data set every time you refer to a variable in it, you can *attach* it.  This practice is generally frowned upon, but is a useful crutch while learning how to use R.  
...
hist(lifeExp)
```
If we use *ggplot* (this is in the *tidyverse* package we already installed), we can get something much more detailed and visually appealing.
```{r}
...
```
We will come back to doing visualizationsa bit later.
...
The keyboard shortcut for %>% is Control-Shift-M.  That's a useful one to know!
...
```
These results indicate that, if GDP per capita goes up by $1000, life expectancy is expected to rise by roughly .6 years.  As we will soon, see, this relationship is not terribly linear, so it might make sense to do a log transformation of the GDP variable.
